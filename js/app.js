...
const ICONS = [
  ...
  { g: "accounts", k: "cash", img: "cash_1.png", l: "Cash_1" },
  { g: "accounts", k: "cash", img: "cash_3.png", l: "Cash_3" },
  { g: "accounts", k: "cash", img: "coins.png", l: "Cash_4" },
...
function setupModalShell(modalId) { const modal = $(modalId); if (modal) modal.classList.toggle('mobile-sheet', isMobileView()); }
function openModal(id) { const el = $(id); if (!el) return; setupModalShell(id); el.classList.remove('hidden'); document.body.classList.add('modal-open'); }
function closeModal(id) { const el = $(id); if (!el) return; el.classList.add('hidden'); document.body.classList.remove('modal-open'); }
function closeAllModals() { document.querySelectorAll('.modal-backdrop').forEach(m => m.classList.add('hidden')); document.body.classList.remove('modal-open'); }
...
function monthTxns() { const ym = new Date().toISOString().slice(0,7); return transactions.filter(t => t.date && t.date.startsWith(ym)); }
...
function renderIconGrid() {
  ...
  $('iconGrid').innerHTML = items.map(x => `<button type="button" class="icon-chip" data-icon="${ICON_BASE}${x.img}"><span class="ico"><img src="${ICON_BASE}${x.img}" alt="${x.l}"></span><span class="lbl">${x.l}</span></button>`).join('');
  $('iconGrid').querySelectorAll('.icon-chip').forEach(b => b.onclick = () => { if (iconTargetInput) iconTargetInput.value = b.dataset.icon; closeModal('iconPickerModal'); });
}
window.openIconPicker = function(id) {
  iconTargetInput = $(id);
  if (!iconTargetInput) return;
  $('iconSearch').value = '';
...
function renderDashboard() {
  ...
  $('dashAccountsList').innerHTML = accounts.map(a => `<div class="txn-item"><div class="ico-circle">${iconHTML(a.emoji, '💳')}</div><div class="txn-meta"><div class="txn-cat">${a.name}</div><div class="txn-sub">${a.type}</div></div><div class="txn-right"><div>${fmt(accountBalance(a.id))}</div></div></div>`).join('') || '<div class="empty-state">No accounts yet.</div>';
}
function renderAccounts() {
  $('accountsList').innerHTML = accounts.length ? accounts.map(a => `<article class="account-card"><div class="ico-circle">${iconHTML(a.emoji, '💳')}</div><div><strong>${a.name}</strong><div class="txn-sub">${a.type}</div></div><div style="margin-left:auto;text-align:right"><div class="kpi-value">${fmt(accountBalance(a.id))}</div></div><div class="row-actions" style="margin-top:8px"><button class="btn-sm" onclick="editAccount('${a.id}')">Edit</button><button class="btn-danger" onclick="deleteAccount('${a.id}')">Delete</button></div></article>`).join('') : '<div class="empty-state">No accounts yet.</div>';
}
...
function renderReimburse() {
  $('reimburseList').innerHTML = reimbursements.length ? reimbursements.map(x => {
    ...
    return `<div class="reimb-item"><div class="card-head"><div><strong>👤 ${x.person_name}</strong><div class="txn-sub">Total: ${fmt(x.total_amount)} · Paid: ${fmt(x.paid_back||0)} · Due: ${fmt(due)}</div></div><span class="badge ${badgeCls}">${x.status}</span></div><div class="progress-bar-wrap"><div class="progress-bar" style="width:${pct}%"></div></div><div class="row-actions">${x.status!=='settled' ? `<button class="btn-sm" onclick="openPayback('${x.id}')">Record payback</button>` : ''}<button class="btn-danger" onclick="deleteReimburse('${x.id}')">Delete</button></div></div>`;
...
function renderReports() {
  ...
  $('reportCatList').innerHTML = Object.entries(map).sort((a,b)=>b[1]-a[1]).map(([cid,amt]) => { const cat = categories.find(c => c.id===cid); const pct = Math.round((amt/total)*100); return `<div class="txn-item"><div class="ico-circle">${iconHTML(cat?.emoji, '📂')}</div><div class="txn-meta">${cat?.name||'Other'}<div class="progress-bar-wrap" style="margin-top:4px"><div class="progress-bar" style="width:${pct}%"></div></div></div><div class="txn-right">${fmt(amt,s)}</div></div>`; }).join('') || '<div class="empty-state">No expense data this month.</div>';
}
function renderSettings() {
  renderProfile();
  $('categoryList').innerHTML = categories.length ? categories.map(c => `<div class="txn-item"><div class="ico-circle">${iconHTML(c.emoji, '📂')}</div><div class="txn-meta"><div class="txn-cat">${c.name}</div><div class="txn-sub">${c.type}</div></div>${!c.is_default ? `<button class="btn-danger" style="margin-left:auto" onclick="deleteCat('${c.id}')">✕</button>` : ''}</div>`).join('') : '<div class="empty-state">No categories yet.</div>';
}
...
function bindUI() {
  ...
  document.querySelectorAll('[data-icon-group]').forEach(b => b.addEventListener('click', () => { document.querySelectorAll('[data-icon-group]').forEach(x => x.classList.remove('active')); b.classList.add('active'); renderIconGrid(); }));
  document.querySelectorAll('[data-ctype]').forEach(btn => btn.addEventListener('click', () => { document.querySelectorAll('[data-ctype]').forEach(x => x.classList.remove('active')); btn.classList.add('active'); currentCatType = btn.dataset.ctype; }));
  document.querySelectorAll('[data-import-kind]').forEach(b => b.addEventListener('click', () => { document.querySelectorAll('[data-import-kind]').forEach(x => x.classList.remove('active')); b.classList.add('active'); currentImportKind = b.dataset.importKind; buildMappingGrid(); }));
  document.querySelectorAll('#txnTypeTabs .type-tab').forEach(btn => btn.addEventListener('click', () => {
    document.querySelectorAll('#txnTypeTabs .type-tab').forEach(x => x.classList.remove('active'));
  ...
  $('previewImportBtn')?.addEventListener('click', doPreview);
  ...
  $('quickAddBtn')?.addEventListener('click', openTxnModal);
  $('fabBtn')?.addEventListener('click', openTxnModal);
  ...
  $('addReimburseBtn')?.addEventListener('click', () => {
  ...
  $('saveReimburseBtn')?.addEventListener('click', async () => {
  ...
  $('savePaybackBtn')?.addEventListener('click', async () => {
  ...
  $('addLedgerPersonBtn')?.addEventListener('click', () => toast('Ledger coming soon'));
  $('addRecurringBtn')?.addEventListener('click', () => toast('Recurring coming soon'));
